...
namespace scan_planner
{
  ...
  void BsplineOptimizer::setParam()
  {
    ...
    lambda1_ = 1.0;    // 平滑 - 关键
    lambda2_ = 1000;//1.0;     // 碰撞
    lambda3_ = 0.1;     // feasibility
    lambda4_ = 1.0;//1.0;
    // 地图已经按机器人 URDF footprint 做了膨胀，这里只保留额外安全距离。
    // 原 1.0 m 会在实机窄空间/楼梯边缘过度保守，容易把目标点判成障碍。
    dist0_   = 0.35;
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
      ...
      for (double a = 1.0; a >= 0.0; a -= step_size)
      {
        ...
        if (flag_got_start && flag_got_end)
        {
          ...
          if (in_id >= 0 && out_id >= in_id && out_id < init_points.cols())
            ...
            printf("Skip invalid collision segment: in_id=%d, out_id=%d \n", in_id, out_id);
        }
      }
    }
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      if (ret == ASTAR_RET::SUCCESS)
      {
        ...
        if (path.size() < 2)
        {
          ...
          printf("[bspline_opt] A-star path has fewer than 2 points \n");
          return a_star_paths;
        }
      ...
      else
      {
        ...
        printf("[bspline_opt] A-star failed; aborting optimization \n");
        return a_star_paths;
      }
    }
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      if (i >= a_star_paths.size() || a_star_paths[i].size() < 2)
      {
        ...
        printf("[bspline_opt] Skip invalid A-star path while assigning control point directions \n");
        continue;
      }
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        if (j < 0) // fail to get the obs free point
        {
          ...
          printf("[bspline_opt] The robot is inside an obstacle \n");
          in_id = 0;
        }
        ...
        if (j >= cps_.size) // fail to get the obs free point
        {
          ...
          printf("[bspline_opt] Trajectory terminal point is in an obstacle; skip this plan \n");
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        if (ret == ASTAR_RET::SUCCESS)
        {
          ...
          if (path.size() < 2)
          {
            ...
            printf("[bspline_opt] A-star path has fewer than 2 points; drop collision segment \n ");
        ...
        else if (ret == ASTAR_RET::SEARCH_ERR && i + 1 < segment_ids.size())
        {
          ...
          printf("[bspline_opt] A-star failed on a collision segment; merge it with the next segment \n");
        }
        else
        {
          // RCLCPP_ERROR(rclcpp::get_logger("bspline_opt"), "A-star error");
          printf("[bspline_opt] A-star error \n");
          segment_ids.erase(segment_ids.begin() + i);
          i--;
        }
      }
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        if (i >= a_star_paths.size() || a_star_paths[i].size() < 2)
        {
          ...
          printf("[bspline_opt] Skip invalid A-star path while assigning rebound directions \n");
          continue;
        }
        ...
        if (got_intersection_id >= 0)
        {
          ...
          printf("[bspline_opt]  Failed to generate rebound direction \n");
      }
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
        ...
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          ...
          if (flag_occ)
          {
            ...
            if (t <= bspline_interval_) // First 3 control points in obstacles!
            {
              ...
              printf("[bspline_opt]  First three control points are in obstacles; t=%f \n",t);
              return false;
            }
      ...
      {
        printf("Solver error. Return=%d, %s; skip this plan", result, lbfgs::lbfgs_strerror(result));
        // while (ros::ok());
      }
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      {
        printf("Solver error while refining: return=%d, %s", result, lbfgs::lbfgs_strerror(result));
      }
